...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const coordinates1 = (x2 - x1) ** 2;
  const coordinates2 = (y2 - y1) ** 2;

  return Math.sqrt(coordinates1 + coordinates2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sumVect = x1 * x2 + y1 * y2;
  const moduleVec1 = Math.hypot(x1, y1);
  const moduleVec2 = Math.hypot(x2, y2);
  return Math.acos((sumVect / moduleVec1) * moduleVec2);
}
...
function getSumOfDigits(num) {
  const arr = Array.from(String(num)).map((it) => +it);
...
function getCountOfOddNumbers(number) {
  const numAbs = Math.abs(number) / 2;
  return Math.ceil(numAbs);
}
